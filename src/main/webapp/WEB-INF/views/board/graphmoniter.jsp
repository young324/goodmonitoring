...
<c:choose>
	<c:when test="${!empty sessionScope.user}">  
		<ul>
			<label>최근 지원하셨던 <a class="nav-link">${C_NAME}</a>의 모집정보를 확인해보세요 </label><a href="/board/fitlistC_NAME">    >바로가기</a>
		</ul>	
		<ul>
...
<c:forEach items="${listTarget}" var="target" varStatus="status" >  
	<button type="button" id="btn_${status.index}" class="btn btn-default" 
	style="width: 100%;" name="tgbtn" value="${target.TARGET}" onClick="monthlybtn();">${target.TARGET}   <div style="display:inline;" id="count">00</div>건</button><br>      
</c:forEach>
 ...
 -->
				...
				<h4><strong>
					<label id="tgtext"></label></strong>
					 대상의 모니터링 모집 정보
					 <strong>
					<label id="tgcounttext"></label></strong> 
...
<div class="row">
	<div class="col-lg-12">
    	<div class="panel panel-default">
    		<div class="panel-heading">모집정보
    		<c:choose>
				<c:when test="${!empty sessionScope.user}">       		
        			<button id='FitBtn' type="button" class="btn btn-xs pull-right">맞춤모집정보</button>        	
        			<button id='listBtn' type="button" class="btn btn-xs pull-right">전체 모집정보</button>          	
			...
			success : function(data){
				...
				listCount = data;
				 ...
				 for(var i=0; i<listCount.length; i++){
					 console.log(listCount[i].interest_category); // Qna객체에 있는 category 필드를 추출해서 alert창에 띄우기
				}     
				/*         
				for (var i=0;i<list.length;i++){
					for (var j=0; j<list.length; j++){
				...
				/* for(var i=0; i<showCount.length; i++){
					 console.log(showCount[i].tcount); // Qna객체에 있는 category 필드를 추출해서 alert창에 띄우기
				} */
